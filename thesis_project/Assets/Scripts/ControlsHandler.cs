...
=======
	...
	public class ControlsHandler : MonoBehaviour {
		...
		void Start () {
			...
			egm = EditorGameManager.Instance;
			sm = SimulationManager.Instance;
			selectionMenu = GameObject.Find ("Menu_Selection");
			...
			if(sm.SelectedController == SimulationManager.Controller.wiiremote) {
				receiver = new WiimoteReceiver();
				receiver.connect();
				//to ensure that wiiremote is instantiated properly
				while(true)
				{
					if(SetWiiMote ()) {
						return;
...
//			KeyboardControls();
	...
	private void KeyboardKeyBinds() {
		if (Input.GetKeyDown (KeyCode.Space)) {
			egm.ActivateSelectionMenu = !egm.ActivateSelectionMenu;
			selectionMenu.GetComponent<SelectionMenu>().DisplaySelectionMenu(egm.ActivateSelectionMenu);
		}
	}
}
>>>>>>> Stashed changes
